            ...
            if existing_user:
                ...
                if existing_user.is_active:
                    ...
                    try:
                        if is_phone:
                            try:
                                eskiz_api.send_sms(identifier, message=_("Workout ilovasiga ro'yxatdan o'tish uchun tasdiqlash kodi:{code}").format(
                ...
                try:
                    if is_phone:
                        eskiz_api.send_sms(
                            identifier,
                            message=_("Workout ilovasiga ro'yxatdan o'tish uchun tasdiqlash kodi:{code}").format(code=verification_code),
                        )
                    else:
                        # Validate email format
                        validator = EmailValidator()
...
class VerifyCodeView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Verify the user with a given code",
        request_body=VerifyCodeSerializer,
        responses={200: "Verification successful. You can now complete your profile."}
    )
    def post(self, request):
